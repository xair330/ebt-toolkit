"""
B777 货机 EBT 分析入口
独立使用 cargo_config.json，不影响 A330/A350 的 ebt_config.json
"""
import os
import sys
...
def main():
    ...
    if not os.path.exists(DATA_FILE):
        ...
        print("请在 cargo_config.json 中配置正确的 DATA_FILE 路径，或拖拽文件到本程序。")
        pause_and_exit()
    ...
    print("[6/7] 图5: B777 胜任力雷达图...")
    radar_data = {}
    sub = cat_df[cat_df["机型"] == "B777"]
    counts = [sub[sub["胜任力"] == c].shape[0] for c in comp_codes]
    total = sum(counts) or 1
    radar_data["B777"] = [x / total for x in counts]
    plot_aircraft_radar(radar_data, comp_codes, os.path.join(OUTPUT_DIR, "图5_B777雷达图.png"))
    ...
    report = f"""# {cfg['REPORT_PERIOD']} B777货机 EBT胜任力分析报告
...
> 详细数据见同目录 CSV 文件
"""
    with open(os.path.join(OUTPUT_DIR, "分析总报告.md"), "w", encoding="utf-8") as f:
        f.write(report)
